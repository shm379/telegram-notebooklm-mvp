...
- **Import بکاپ تلگرام**: خروجی **JSON** یا **HTML** تلگرام دسکتاپ (`result.json`/`messages.html` یا یک `.zip` از پوشه‌ی export) را — هم تک‌چت و هم export کامل اکانت — می‌گیرد، همه‌ی پیام‌ها را قابل‌جستجو می‌کند و یک نسخه‌ی **Markdown** برمی‌گرداند. مدیای داخل zip (عکس/صوت/ویدیو) با OCR/transcription به متن تبدیل می‌شود. در ربات کافی است فایل را بفرستید؛ در وب از کارت «Import Telegram Backup» آپلود کنید.
- **سایت لندینگ + وب‌اپ چندکاربره**: صفحه‌ی معرفی روی `/` و داشبورد کامل (Account / Ingest / Search / Ask / Library / Sources / Import بکاپ) روی `/app`؛ با حساب کاربری اختیاری، هر کاربر آرشیو خصوصی خودش را دارد
...
۲. فرمت **JSON** (Machine-readable) یا **HTML** را انتخاب کنید — هر دو پشتیبانی می‌شوند.
۳. خروجی یک فایل `result.json` یا `messages.html` (یا پوشه‌ای شامل آن به‌همراه مدیا) است؛ می‌توانید پوشه را zip کنید.

سپس:

- **از ربات:** فایل `result.json`، `messages.html` یا `.zip` را مستقیماً برای ربات بفرستید. ربات آن را import می‌کند، محتوا را قابل‌جستجو می‌کند و یک نسخه‌ی Markdown برمی‌گرداند. (سقف ۲۰ مگابایت به‌خاطر محدودیت دانلود Bot API؛ برای فایل بزرگ‌تر از وب استفاده کنید.)
- **از وب:** در `/app` کارت «Import Telegram Backup»، فایل را آپلود کنید. محتوا قابل‌جستجو می‌شود و دکمه‌ی دانلود Markdown ظاهر می‌شود. با دکمه‌ی «Convert only» می‌توانید فقط Markdown بگیرید بدون ایندکس.

اگر بکاپ یک `.zip` همراه مدیا باشد، عکس‌ها با OCR و صوت/ویدیو/voice با transcription (در صورت وجود کلید Gemini) به متن قابل‌جستجو تبدیل می‌شوند. هر چت به یک منبع مصنوعی `backup://<id>` تبدیل می‌شود و import به‌صورت idempotent است (وارد کردن دوباره‌ی همان فایل چیزی اضافه نمی‌کند).
...
http://127.0.0.1:8000/app    # داشبورد: Account / Ingest / Search / Ask / Library / Sources / Import بکاپ
```

در `/app` می‌توانید **حساب کاربری** بسازید (Register/Login) تا آرشیو خصوصی و ایزوله بگیرید؛ بدون ورود، روی workspace مشترک (محافظت‌شده با `WEB_API_TOKEN`/loopback) کار می‌کنید. session با کوکی `tgnb_session` نگه داشته می‌شود و رمزها با PBKDF2 هش می‌شوند.
