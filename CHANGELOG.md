...
## Fix: keyword search matched the whole query as one substring (2026-06-19)

رفع باگی که باعث می‌شد `/search` و `/ask` برای عبارت‌های چندکلمه‌ای و سؤال‌ها «چیزی پیدا نشد» بدهند.

### Problem
- `keyword_candidates` کل عبارت جستجو را به‌صورت یک `LIKE '%...%'` واحد match می‌کرد. پس `/search فرشاد کیه؟` هیچ‌وقت match نمی‌شد چون هیچ پیامی عین این عبارت را ندارد — هرچند «فرشاد» ده‌ها بار در آرشیو هست. عملاً فقط جستجوی تک‌کلمه‌ایِ دقیق کار می‌کرد.

### Fix
- ماژول خالص `keyword_search.query_terms(query)`: عبارت را به کلمات می‌شکند (Unicode-aware؛ علائم نگارشی مثل ؟ ، . حذف می‌شوند)، کلمات تک‌حرفی و تکراری را drop می‌کند.
- `keyword_candidates` حالا با `OR` روی هر کلمه match می‌کند و نتایج را بر اساس **تعداد کلمات منطبق** رتبه‌بندی می‌کند (over-fetch سپس trim به `top_k`). برای query بدون کلمه‌ی قابل‌استفاده، به همان رفتار قبلی (whole-query substring) برمی‌گردد.
- روی دیتای موجود بلافاصله اثر دارد (فقط منطق زمانِ جستجو عوض شده؛ نیازی به re-import نیست). `/ask` هم چون از همین جستجو به‌عنوان context استفاده می‌کند، بهتر می‌شود.

### Tests
- `tests/test_keyword_search.py`: توکنایز (شکستن/حذف علائم/drop کوتاه/dedupe/سقف)، بازتولید دقیق باگ گزارش‌شده («فرشاد کیه؟» → پیام حاوی «فرشاد»)، رتبه‌بندی بر اساس تعداد کلمات منطبق، و دست‌نخوردگی رفتار تک‌کلمه‌ای.
